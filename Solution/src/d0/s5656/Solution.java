 ...
 * - 0인 곳을 위에서 내려와서 채워줄 필요 있음
 * 
 * 가지치기 : 중간에 블럭이 다 없어지면 더 이상 가볼 필요가 없다.
 * --> 순열이 만들어지는 과정에서 그만둘 수 있도록 처리
 */
...
//			3. 구슬을 떨어뜨려서 벽돌을 깬다 --> 깨진 벽돌의 수?
			int broken = crash(first, cloned);
//			3-1. 이미 벽돌이 다 깨졌으면 정답 0, 종료
			if (broken >= brickCnt) {
				answer = 0;
				return;
			}
